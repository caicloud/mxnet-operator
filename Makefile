...
ROOT := github.com/kubeflow/mxnet-operator

# Target binaries. You can build multiple binaries for a single project.
TARGETS := mxnet-operator.v1
...
REGISTRY ?= lever-dev.cargo.io/release
...
container: build-linux
	image=$(IMAGE_PREFIX)mxnet-operator$(IMAGE_SUFFIX);                                  \
	docker build -t $(REGISTRY)/$${image}:$(VERSION)                                 \
	    --label $(DOCKER_LABELS)                                                       \
	    -f $(BUILD_DIR)/mxnet-operator.v1/Dockerfile .;
